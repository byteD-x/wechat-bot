...
这个项目的 RAG 不是“只要检索命中了就塞进 Prompt”。当前实现已经有可选 Hybrid Search 和两层精排：

- Hybrid Search + Query Rewrite：规则化改写用户问题，融合向量候选与本地关键词候选，默认关闭，开启后仍复用同一套 citation 和重排链路
- 轻量重排：向量距离 + 关键词重合
...
- 便携版更新提示：保留 GitHub Release 版本检查与弹窗提示，但通过手动更新模式阻断应用内下载和安装，引导用户打开 Releases 手动替换
...
- 只读 MCP adapter 通过 `POST /api/v1/mcp` 落地，复用 `ControlledToolWorkflowService` 与模型侧安全白名单，只支持 `initialize`、`tools/list`、`tools/call`，不暴露 `prompt_preview`、`config_audit`、resources、prompts 或动态插件。
...
- Electron 主进程只转发固定治理路径；Prompt 回滚必须匹配数字 revision，Tool Workflow 与 MCP adapter 不支持任意 shell、文件写入、网络请求或动态插件执行，维护 dry-run 只返回聚合摘要，不展示备份候选列表、清理 targets 或完整本机路径，也不影响微信主回复或快回复链路。
...
- 知识库治理接口已提供 `status / index / auto-index preview / auto-index jobs / dry-run / batch-dry-run / ingest / batch-ingest / rebuild / batch-rebuild / jobs / delete` 本机 API，复用现有 `KnowledgeBaseService`、`KnowledgeBaseJobQueue` 和运行时 `vector_memory`，不另起一套索引存储。
- 写入类端点只接收请求体中的纯文本或 Markdown，不读取任意文件路径、不开放上传管道；`dry-run` 和 `batch-dry-run` 只返回 chunk 数量、chunk id、字符数和脱敏来源摘要，不回显正文。
- `auto-index/preview` 只扫描固定 `data/knowledge_base/inbox` 一层目录中的 `.txt/.md/.markdown` 文本，返回 dry-run 文件摘要和跳过原因；`auto-index/jobs` 只把同一固定 inbox 当前可导入文档以 `rebuild` 模式提交到现有后台队列。两者都不递归、不展开 glob、不接受任意路径、不返回正文、chunk text、embedding 或完整本机路径。
...
- `jobs` 支持请求体单文档或 `{"documents":[...]}` 批量文档异步 `ingest/rebuild`，使用进程内内存级串行队列；`status.queue` 和 `GET /api/knowledge_base/jobs/{job_id}` 只返回状态、事件时间线、聚合结果与脱敏文档摘要，不持久化任务，不读取 `source_file`，不返回正文、chunk text、embedding 或完整本机路径。
...
- 设置页已支持刷新状态、单文档预览/写入/重建、固定 inbox 预览后受控入队，以及受控 `{"documents":[...]}` JSON 批量预览/写入/重建；单文档可手动粘贴纯文本 / Markdown，或通过固定桌面 IPC 显式选择单个 `.txt/.md/.markdown` 文件填入表单，来源只保留 `.../<filename>`；写入、重建或固定 inbox 入队前都必须先完成对应 dry-run，内容或元数据变化后会强制重新预览。
- 本机 CLI `python run.py knowledge-base import-files` 已提供显式 `.txt/.md` 文件列表入口，默认只预览分块；拒绝目录和 glob，`--apply` 才调用 loopback 本机 API 写入。`python run.py knowledge-base import-inbox` 复用固定 inbox 的预览与受控入队边界，默认只 dry-run，`--apply` 才把固定 inbox 文档提交到本机受控队列。
...
- 本文作为项目亮点入口，适合展示差异化定位、产品化能力和长期运行治理。
- 技术难题与 STAR 复盘优先引用 `docs/STAR_REPO_STAR.md`，避免在亮点文档里重复展开面试案例。
- 简历中可写的指标以本文和 `STAR_REPO_STAR.md` 的证据索引为准；回复成功率、平均响应时延、Star/Fork 等真实运营指标仍需补充。
