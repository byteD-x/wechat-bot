...
- 设置页单文档入口已支持刷新状态、预览分块、写入知识库和重建同文档；可手动粘贴纯文本 / Markdown，或通过固定桌面 IPC 显式选择单个 `.txt/.md/.markdown` 文件填入表单，来源只保留 `.../<filename>`；写入或重建前都必须先对当前内容完成 dry-run，内容或元数据变化后会强制重新预览。
