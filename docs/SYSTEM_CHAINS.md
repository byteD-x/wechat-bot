...
12. `/api/knowledge_base/*`
    - 功能：提供本机知识库治理闭环，支持查看状态、预览分块、写入、重建同文档和按 `doc_id` 删除。
    - 实现：
      ...
      - 当前设置页不开放 `delete`、文件上传、目录扫描或任意本机路径读取；Web API 也只接收请求体中的纯文本或 Markdown。
...
- 知识库治理：`/api/knowledge_base/status`、`/api/knowledge_base/dry-run`、`/api/knowledge_base/ingest`、`/api/knowledge_base/rebuild`、`/api/knowledge_base/delete`
