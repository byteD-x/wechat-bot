...
- 知识库治理 API：`GET /api/knowledge_base/status` 与 `POST /api/knowledge_base/dry-run|batch-dry-run|ingest|batch-ingest|rebuild|delete` 已落地，首版只支持请求体 text/Markdown，不读取任意本机文件，预览不返回正文或完整本机路径；`batch-dry-run` 仅做最多 20 份请求体文档的无副作用分块预览，`batch-ingest` 仅按顺序写入请求体文档，不批量重建或删除旧 chunk。
- 知识库治理 UI 最小入口：设置页“数据与恢复 / 知识库治理”已支持粘贴纯文本或 Markdown、刷新状态、预览分块，并要求同一份内容 dry-run 后才允许写入或重建同文档；暂不开放文件上传、目录扫描或 delete。
...
- RAG 知识库治理增强
  - 在当前粘贴式 UI、受控同文档重建、显式文件 CLI、Web API 批量预览和 Web API 批量写入之上，继续评估后台队列、文件选择器和批量重建；继续保持不开放任意路径扫描。
