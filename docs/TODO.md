...
- 知识库治理 CLI 入口：`python run.py knowledge-base import-files` 已支持 `.txt/.md` 显式文件列表，默认只 dry-run，拒绝目录和 glob，`--apply` 才调用 loopback 本机 API 写入；`python run.py knowledge-base import-inbox` 已支持固定 inbox 只读预览与 `--apply` 受控入队。
...
- RAG 知识库治理增强
  - 在当前粘贴 / 显式单文件选择 UI、受控同文档重建、受控批量治理 UI、显式文件 CLI、固定 inbox CLI 受控入队、已入库索引摘要、Web API 批量预览、批量写入、批量重建和请求体后台队列之上，继续评估是否把固定 inbox 能力接入桌面设置页；继续保持不开放任意路径扫描。
