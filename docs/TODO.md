...
- 知识库治理 API：`GET /api/knowledge_base/status` 与 `POST /api/knowledge_base/dry-run|batch-dry-run|ingest|rebuild|delete` 已落地，首版只支持请求体 text/Markdown，不读取任意本机文件，预览不返回正文或完整本机路径；`batch-dry-run` 仅做最多 20 份请求体文档的无副作用分块预览，不写入向量库。
...
- RAG 知识库治理增强
  - 在当前粘贴式 UI、受控同文档重建、显式文件 CLI 和 Web API 批量预览之上，继续评估后台队列、文件选择器、批量写入和批量重建；继续保持不开放任意路径扫描。
