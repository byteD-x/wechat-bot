...
- API 测试：已覆盖 Prompt 版本列表、diff 预览、空/损坏账本诊断、active revision 唯一性、Prompt 回滚成功、revision 不存在、白名单工具执行、只读评测/成本/维护 dry-run 工具、`plan_reflect_repair` 安全修复、未知工具拒绝和危险 payload 拒绝。
