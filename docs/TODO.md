...
- RAG 知识库治理增强
  - 在当前粘贴 / 显式单文件选择 UI、固定 inbox 预览后受控入队、受控同文档重建、受控批量治理 UI、显式文件 CLI、固定 inbox CLI 受控入队、已入库索引摘要、Web API 批量预览、批量写入、批量重建和请求体后台队列之上，继续评估知识库治理的可观测性和 badcase 分析；继续保持不开放任意路径扫描。
