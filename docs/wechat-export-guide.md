...
如果你把导出语料 RAG、联系人专属 Prompt 或系统 Prompt 调整到不理想的状态，请优先用项目内的配置备份和 Prompt 治理能力恢复；Prompt 回滚接口会追加新的 active revision，不会覆盖历史账本。完整接口边界见 [API 契约与治理接口](api.md)。
...
5. 排障时优先导出自动脱敏的诊断支持包；如果需要提供完整日志或 CSV，必须先确认没有聊天正文、联系人真实标识和敏感路径外泄。
