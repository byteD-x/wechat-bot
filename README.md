...
- `Knowledge Base Governance`: 本地 Web API 已提供知识库文档 `dry-run / batch-dry-run / ingest / batch-ingest / rebuild / batch-rebuild / delete / status` 最小闭环，首版只接收请求体中的纯文本或 Markdown，不读取任意本机路径，预览响应只返回 chunk 摘要和脱敏来源；`batch-dry-run` 仅做最多 20 份请求体文档的无副作用预览，`batch-ingest` 仅顺序写入请求体文档，`batch-rebuild` 会按顺序重建请求体文档、拒绝重复 `doc_id`，并在单文档 embedding 准备失败时保留该文档旧 chunk；桌面设置页支持手动粘贴，或通过固定 IPC 显式选择单个 `.txt/.md/.markdown` 文件填入表单，来源只保留 `.../<filename>`，仍需同一份内容 dry-run 后才能写入或重建同文档；`python run.py knowledge-base import-files` 提供显式文件列表 CLI，默认只预览，`--apply` 才调用运行中的本机 API 写入。
...
> 知识库 UI 说明：设置页已经提供单文档治理入口，支持手动粘贴纯文本 / Markdown，或显式选择单个 `.txt/.md/.markdown` 文件把内容填入表单；选择文件不会上传、不会扫描目录、不会返回完整本机路径，也不会自动写入或重建，仍需先对当前内容完成 dry-run；批量预览、批量写入、批量重建和 delete 仍未在桌面设置页开放。
...
"agent": {
    ...
    "retriever_hybrid_enabled": false,        # 开启后融合向量召回与本地关键词召回
