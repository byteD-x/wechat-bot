...
> 知识库 UI 说明：设置页已经提供单文档治理入口，支持手动粘贴纯文本 / Markdown，或显式选择单个 `.txt/.md/.markdown` 文件把内容填入表单；也提供 `{"documents":[...]}` 受控 JSON 批量入口。选择文件不会上传、不会扫描目录、不会返回完整本机路径，也不会自动写入或重建；固定 inbox 预览目前接入 Web API 与本机 CLI，尚未接入桌面设置页；单文档、批量和固定 inbox 写入都必须先完成对应 dry-run；delete 仍未在桌面设置页开放。
